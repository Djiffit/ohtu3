...
public class App {
    ...
    public void run() {
        while (true) {
            ...
            if (checkForEmpty(command)) break;
            checkInput(command);
        }
    }
    ...
    public boolean checkForEmpty(String command) {
        return command.isEmpty();
    }

    public void newUserFunction() {
        String[] usernameAndPasword = ask();
        if (usernameAndPasword.length == 2 && auth.createUser(usernameAndPasword[0], usernameAndPasword[1])) {
